...
class FMBlock(nn.Module):
    def __init__(self, nf=64, opt=None):
        ...
        self.down = nn.Conv2d(nf, opt['FM_channelNumber'], 1, 1, 0, bias=True)  # 64 -> 16
        self.conv1_f = nn.Conv2d(opt['FM_channelNumber'], 2 * opt['FM_kernelNumber'], 3, 1, 1, bias=True)  # 16 -> 8
        self.conv1_w = nn.Conv2d(opt['FM_channelNumber'], opt['FM_kernelNumber'], 3, 1, 1, bias=True)  # 16 -> 4
        self.conv2 = nn.Conv2d(opt['FM_channelNumber'], opt['FM_channelNumber'], 3, 1, 1, bias=True)  # 16 -> 4
        self.up = nn.Conv2d(opt['FM_channelNumber'], nf, 1, 1, 0, bias=True)  # 16 -> 64
